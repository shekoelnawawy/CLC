...
def robust_accuracy_aa(test_loader, proxy_model, undefended_model, eps, batch_size, device):
  ...
  # Explicitly initialize CUDA if using GPU to ensure it's available for AutoAttack
  # AutoAttack checks CUDA availability internally, so we need to ensure CUDA is initialized
  # before AutoAttack tries to use it. Creating a dummy tensor forces CUDA initialization.
  if device == 'cuda':
    if not torch.cuda.is_available():
      raise RuntimeError("CUDA device requested but CUDA is not available!")
    # Force CUDA initialization by creating a dummy tensor on GPU
    # This ensures CUDA is properly initialized before AutoAttack tries to use it
  ...
  auto_attack = AutoAttack(bpda_forward, norm='Linf', eps=eps, verbose=True, seed=999, device=device)
...
def main() -> None:
  ...
  if torch.cuda.is_available():
    ...
    # Explicitly initialize CUDA early to ensure it's ready for AutoAttack
    _ = torch.zeros(1).cuda()
    print("CUDA initialized successfully.")
  else:
    print("WARNING: CUDA is not available! Running on CPU (this will be very slow).")
    print("Make sure:")
    print("  1. NVIDIA drivers are installed")
    print("  2. Docker has GPU access (--gpus all flag)")
  ...
  cifar10_1k = datasets.CIFAR10(root='./data', train=False, download=True, transform=transforms.Compose([
        transforms.ToTensor(),
