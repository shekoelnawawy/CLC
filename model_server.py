...
def main():
  ...
  # Compute the projections. Use GPU if available, otherwise CPU.
  # Note: GPU is faster but needs more VRAM. CPU is slower but uses system RAM.
  print("Learning linear projections, this could take a while (10min+)...")
  if device == 'cuda':
    print("Computing projections on GPU (faster but needs more VRAM)...")
    proj = compute_projection_matrices(model, train_loader, max_num_samples=max_num_samples_for_proj, device=device)
  else:
    print("Computing projections on CPU (slower but uses system RAM)...")
    model = model.to("cpu")
